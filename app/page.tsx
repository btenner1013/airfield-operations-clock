...
export default function Home() {
  ...
  const sceneStyle={...cloudStyle,"--obsc-opacity":obscuration.density,"--obsc-duration":`${obscuration.duration}s`,"--obsc-direction":obscuration.direction} as unknown as CSSProperties;
  ...
  return <main className={`display theme-${condition} phase-${phase}`} style={sceneStyle} data-coverage={effCoverage} data-tier={cloudTierV} data-perf={perf} data-performance={perf} data-base={sceneModel.cloudBaseFt??""} data-intensity={fx.intensity} data-vicinity={fx.vicinity?"1":"0"} data-wind={effWindSpd} data-winddir={effWindDir??""} data-vis={effVisibility??""} data-nx={cloudVec.nx} data-ny={cloudVec.ny} data-precip-type={fx.precip} data-secondary-precip-type={fx.secondaryPrecip} data-precip-intensity={fx.intensity} data-precip-active={fxSpec?"1":"0"} data-particle-count={fxSpec?.totalCount??0} data-primary-particle-count={fxSpec?.count??0} data-secondary-particle-count={fxSpec?.secondary?.count??0} data-obscuration={obscuration.type} data-obscuration-density={obscuration.density.toFixed(2)} data-obscuration-direction={obscuration.direction} data-active-obscuration-layers={obscuration.layers} data-visibility={effVisibility??""} data-reduced-motion={reduced?"1":"0"}>
