...
export default function Home() {
  ...
  return <main className={`display theme-${condition} phase-${phase}`}>
    <div className="sky" aria-hidden="true"><i className="sky-base" style={{backgroundImage:`url(${imageBase}/airfield-${scene}.png)`}}/><i className="cloud c1"/><i className="cloud c2"/><i className="fog-layer"/><i className="weather-fx"/><i className="rain-field">{Array.from({length:42},(_,i)=><span key={i} style={{left:`${(i*37+7)%101}%`,height:`${46+(i*29)%72}px`,animationDelay:`-${((i*31)%29)/10}s`,animationDuration:`${.62+((i*17)%24)/100}s`}}/>)}</i><i className="glass-droplets">{Array.from({length:18},(_,i)=><span key={i}/>)}</i><i className="snow-field">{Array.from({length:30},(_,i)=><span key={i} style={{left:`${(i*43+5)%101}%`,fontSize:`${8+(i*7)%14}px`,animationDelay:`-${((i*19)%71)/10}s`,animationDuration:`${6.5+((i*13)%42)/10}s`}}>❄</span>)}</i><i className="lightning-layer" style={{backgroundImage:`url(${imageBase}/airfield-lightning-overlay.png)`}}/><i className="pavement-reflection"/></div>
