...
function simplifyLightningRemark(raw: string): string {
  ...
  if (/OCNL\s+LTGIC\s+DSNT/i.test(clean) || /DISTANT\s+LIGHTNING/i.test(clean) || /LTG\s+DSNT/i.test(clean)) {
    let dir = "";
    const m = clean.match(/(?:DSNT|LIGHTNING)\s+([N|S|E|W|NE|NW|SE|SW|\/]+)/i) || clean.match(/\b(N|S|E|W|NE|NW|SE|SW)\b/i);
    if (m) dir = m[1].toUpperCase();
    return dir ? `DISTANT LIGHTNING · ${dir}` : "DISTANT LIGHTNING";
  }
  if (/VCTS/i.test(clean)) return "THUNDERSTORMS IN THE VICINITY";
  ...
  return <main ref={mainRef} className={`display theme-${condition} phase-${phase}`} style={sceneStyle} data-wallpaper-scene={scene}>
    ...
    <div className="shade"/><div className="burn-shift">
      ...
      <section className="info">
        ...
        <article className="weather-card panel"><div className="panel-title"><span>CURRENT WEATHER</span><b>{weather.source==="METAR"?"KMEM METAR":CONFIG.locationName.toUpperCase()}</b></div><div className="weather-main"><span className="weather-glyph"><WeatherIcon condition={displayTheme} night={phase==="night"}/></span><strong>{weather.temperatureF}<span className="temp-unit">°F</span></strong><div className="weather-copy"><b>{debug?displayTheme.replace("-"," "):weather.description}{weather.operationalWeather?.secondaryLabel && <span className="weather-modifier"> · {weather.operationalWeather.secondaryLabel}</span>}</b><div className="feels-like-container"><span className="feels-like">FEELS LIKE <strong>{weather.feelsLikeF??weather.temperatureF}°F</strong></span><span className="ceiling-line">CEILING <strong>{weather.cloudCoverage && ["BKN","OVC","VV"].includes(weather.cloudCoverage) && weather.cloudBaseFt !== null ? `${weather.cloudBaseFt.toLocaleString()} FT` : "UNL"}</strong></span><span className="humidity-line">HUMIDITY <strong>{weather.humidity}%</strong></span></div>{lightning.awareness&&<small className="lightning-awareness">REMARK: {simplifyLightningRemark(lightning.awareness)}</small>}</div></div><div className={`metar-health health-${metarState.toLowerCase()}`}><span>METAR {metarState}</span>{feed!=="OK"&&<span className={`feed-${feed.toLowerCase()}`}>FEED {feed}</span>}</div></article>
