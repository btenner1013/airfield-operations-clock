  ...
  return <main className={`display theme-${condition} phase-${phase}`}>
    ...
    <div className="shade"/><div className="burn-shift">
      ...
      <section className="info">
        ...
        <article className="weather-card panel"><div className="panel-title"><span>CURRENT WEATHER</span><b>{CONFIG.locationName.toUpperCase()}</b></div><div className="weather-main"><span className="weather-glyph">{weatherGlyph(displayTheme)}</span><strong>{weather.temperatureF}<span className="temp-unit">°F</span></strong><div><b>{debug?displayTheme.replace("-"," "):weather.description}</b><small>FEELS LIKE {weather.feelsLikeF??weather.temperatureF}° · WIND {weather.windSpeedMph} MPH {weather.windDirection}</small><small>HUMIDITY {weather.humidity}%</small></div></div>{weather.stale&&<span className="stale">WEATHER DATA STALE</span>}</article>
        <article className="forecast-card panel"><div className="panel-title"><span>FUTURE WEATHER · NEXT 9 HOURS</span><b>JULIAN {julian4(now)}</b></div><div className="forecast-grid">{weather.forecast?.length?weather.forecast.map((f,i)=><div key={`${f.time}-${i}`}><time>{f.time}</time><span>{weatherGlyph(f.condition)}</span><small className="forecast-condition">{f.condition.replace("-"," ")}</small><strong>{f.temperatureF}°</strong><small>{f.precipitation}% PRECIP</small></div>):<div className="forecast-empty">FORECAST UNAVAILABLE</div>}</div></article>
      </section>
